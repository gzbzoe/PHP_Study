<?php
	...
	if(in_array('Irix', $array3))
	{
		echo "got Irix";
	}
	if(in_array('Mac', $array3))
	{
		echo "got Mac";
	}
	$array4=array('1.10',12.4,1.13);
	if(in_array('12.4',$array4,false)) echo "got 12.4"; //模糊匹配
	if(in_array('12.4',$array4,true)) echo "got 12.4";//精确匹配
	//4.array_key_exists()判断key是否存在
	$array5=array('first'=>1,
					'second'=>2);
	if(array_key_exists('first', $array5))
		echo "键名first在数组中";
	$array6=array('first'=>null,
					'second'=>2);
	
	echo array_key_exists('first',$array6)?1:2; //查找键
	echo isset($array6['first'])?1:2;//查找键的值
	//5.array_flip() 交换数组的键和值
		...
		echo count($array10);
		echo count($array10,1);//添加参数可以将元素内层的元素进行统计
		//8.统计数组元素的个数 array_count_values()
