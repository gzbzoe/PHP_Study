<?php
	...
	//包含多种数据库  PDO---PHP(各种数据库的驱动程序)
	$dsn="mysql:dbname=bookstore;host=localhost";//mysql
	//$dsn="OCI:dbname=XXX;charset=UTF-8" //Oracle
	$use='root';
	$password='12345678';
	$opt=array(PDO::ATTR_PERSISTENT=>true);//配置长连接
	try{
		$dbh= new PDO($dsn,$use,$password,$opt);
		echo "数据库连接成功！";
	}catch(Exception $e){
	...
	echo "\n连数据库服务版本号信息:".$dbh->getAttribute(PDO::ATTR_SERVER_VERSION);
