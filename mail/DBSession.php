...
class DBSession{
	...
	protected static $ua=null;//用户浏览器信息
	...
	public static function start(PDO $pdo){
		...
		//初始化ip地址
		self::$ip=!empty($_SERVER['HTTP_CLIENT_IP'])?$_SERVER['HTTP_CLIENT_IP']:'unknown';
		//判断ip地址是否合法
		filter_var(self::$ip,FILTER_VALIDATE_IP)===false && self::$ip='unknown';
		//设置SESSION的生理周期 直接从php.ini配置文件中获取生命周期
		self::$lifetime=ini_get("session.gc_maxlifetime");
		//初始化当前时间
		self::$time=time();
		//使用php中提供的自定义session函数完成信息的数据库存储
		//回调函数，每一个参数都是一个函数
		session_set_save_handler( 
			array(__CLASS__,'open'),//程序---可以自动将php.ini信息 
		...
		public static function read($sid){
			$sql='SELECT * FROM session WHRER sid=?';
